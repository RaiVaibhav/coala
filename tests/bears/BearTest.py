...
class BearTest(BearTestBase):
    ...
    def test_profiler_with_no_directory_exists(self, dump_stats):
        ...
        dump_stats.assert_called_once_with('name_TestTwoBear.prof')
        ...
        dump_stats.assert_called_with('name_TestThreeBear.prof')
    ...
    def test_profiler_with_file_path(self):
        ...
        with NamedTemporaryFile() as temp_file:
            ...
            with self.assertRaisesRegex(FileExistsError,
                                        r'(file already exists|File exists)'):
    ...
    def test_profiler_with_debugger(self):
        ...
        with self.assertRaisesRegex(ValueError,
                                    'Cannot run debugger and profiler at the '
                                    'same time.'):
    ...
    def test_is_profiled(self):
        with self.assertRaisesRegex(ValueError,
                                    'Positional argument bear is not an '
        ...
        section.append(Setting('profile', 'tRuE'))
        self.assertEqual(_is_profiled(uut), True)
        section.append(Setting('profile', '0'))
        self.assertEqual(_is_profiled(uut), False)
